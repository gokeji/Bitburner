...
const argsSchema = [
    ...
    ["disable-spending-hashes", false], // Set to true to not spawn hacknet-spend.js to spend hashes on bladeburner
];
...
export async function main(ns) {
    ...
    if (!(7 in ownedSourceFiles))
        return log(
            ns,
            "ERROR: You have not yet unlocked the bladeburner API (need SF7 or to be in BN7)",
            true,
            "error",
        );
    ...
    while (true) {
        try {
            ...
            log(
                ns,
                `WARNING: bladeburner.js Caught (and suppressed) an unexpected error in the main loop:\n` +
        ...
        await ns.sleep(
            Math.min(options["update-interval"], nextTaskComplete > 0 ? nextTaskComplete : Number.MAX_VALUE),
        );
    }
}
...
const getBBDict = async (ns, strFunction, elements, ...args) =>
    await getNsDataThroughFile(
        ns,
        `Object.fromEntries(JSON.parse(ns.args[0]).map(e => [e, ns.bladeburner.${strFunction.replace("%", "e")}]))`,
        `/Temp/bladeburner-${strFunction.split("(")[0]}-all.txt`,
        [JSON.stringify(elements), ...args],
    );
...
async function gatherBladeburnerInfo(ns) {
    ...
    const blackOpsToBeDone = await getBBDictByActionType(
        ns,
        "getActionCountRemaining",
        "Black Operations",
        blackOpsNames,
    );
    remainingBlackOpsNames = blackOpsNames
        .filter((n) => blackOpsToBeDone[n] === 1)
        .sort((b1, b2) => blackOpsRanks[b1] - blackOpsRanks[b2]);
    log(
        ns,
        `There are ${remainingBlackOpsNames.length} remaining BlackOps operations to complete in order:\n` +
    ...
    haveSimulacrum = !(4 in ownedSourceFiles)
        ? true // If player doesn't have SF4, we cannot check, so hope for the best.
        : await getNsDataThroughFile(
              ns,
              `ns.singularity.getOwnedAugmentations().includes("${simulacrumAugName}")`,
              "/Temp/bladeburner-hasSimulacrum.txt",
          );
...
const getMinKeyValue = (dict, filteredKeys = null) =>
    (filteredKeys || Object.keys(dict)).reduce(
        ([k, min], key) => (dict[key] < min ? [key, dict[key]] : [k, min]),
        [null, Number.MAX_VALUE],
    );
const getMaxKeyValue = (dict, filteredKeys = null) =>
    (filteredKeys || Object.keys(dict)).reduce(
        ([k, max], key) => (dict[key] > max ? [key, dict[key]] : [k, max]),
        [null, -Number.MAX_VALUE],
    );
...
async function mainLoop(ns) {
    ...
    if (nextBlackOp == null && !lastBlackOpComplete) {
        const msg =
            `Bladeburner has completed the last BlackOp! (At ${formatDuration(getTimeInBitnode())}). ` +
    ...
    const getCount = (actionName) =>
        contractNames.includes(actionName)
            ? contractCounts[actionName]
            : operationNames.includes(actionName)
              ? operationCounts[actionName]
              : generalActionNames.includes(actionName)
                ? Number.POSITIVE_INFINITY
                : remainingBlackOpsNames.includes(actionName)
                  ? 1
                  : 0;
    ...
    const reservedActions = ["Raid", "Stealth Retirement Operation"].concat(
        populationActions
    ...
    if (getCount("Raid") > 0 && unreservedActions.every((c) => getCount(c) == 0)) {
        ...
        goingRaiding =
            raidableCities.length > 0 &&
            (raidableCities[0] != highestPopCity || options["allow-raiding-highest-pop-city"]);
        if (goingRaiding) {
            // Select the raid-able city with the smallest population
    ...
    if (!goToCity) {
        // Otherwise, cities with higher populations give better operation chances
        ...
        [goToCity, population] = getMaxKeyValue(
            populationByCity,
            acceptableCities.length > 0 ? acceptableCities : cityNames,
        );
        travelReason =
            `Highest population (${formatNumberShort(population)}) city, with chaos ${chaosByCity[goToCity].toFixed(1)}` +
            (acceptableCities.length == 0
                ? ` (all cities above chaos threshold of ${options["chaos-recovery-threshold"]})`
                : "");
    }
    ...
    const contractChances = await getBBDictByActionType(
        ns,
        "getActionEstimatedSuccessChance",
        "Contracts",
        contractNames,
    );
    const operationChances = await getBBDictByActionType(
        ns,
        "getActionEstimatedSuccessChance",
        "Operations",
        operationNames,
    );
    const blackOpsChance =
        nextBlackOp === null || rank < blackOpsRanks[nextBlackOp]
            ? [0, 0] // Insufficient rank for blackops means chance is zero
            : (await getBBDictByActionType(ns, "getActionEstimatedSuccessChance", "Black Operations", [nextBlackOp]))[
                  nextBlackOp
              ];
    // Define some helpers for determining min/max chance for each action
    const getChance = (actionName) =>
        contractNames.includes(actionName)
            ? contractChances[actionName]
            : operationNames.includes(actionName)
              ? operationChances[actionName]
              : generalActionNames.includes(actionName)
                ? [1, 1]
                : nextBlackOp == actionName
                  ? blackOpsChance
                  : [0, 0];
    ...
    const actionSummaryString = (action) =>
        `Success Chance: ${(100 * minChance(action)).toFixed(1)}%` +
        (maxChance(action) - minChance(action) < 0.001 ? "" : ` to ${(100 * maxChance(action)).toFixed(1)}%`) +
        `, Remaining: ${getCount(action)}`;
    ...
    lowStaminaTriggered =
        staminaPct < options["low-stamina-pct"] || (lowStaminaTriggered && staminaPct < options["high-stamina-pct"]);
    ...
    if (lowStaminaTriggered) {
        ...
        chaosByCity[currentCity] > options["chaos-recovery-threshold"] &&
        getCount(antiChaosOperation) > 0 &&
        minChance(antiChaosOperation) > 0.99
    ) {
        ...
        bestActionName =
            getCount(antiChaosOperation) > 0 && minChance(antiChaosOperation) > 0.8 ? antiChaosOperation : "Diplomacy";
    ...
    else if (goingRaiding && maxChance("Raid") > options["success-threshold"]) {
        // Special-case: Ignore min-chance. Population estimate turns bad as we decimate it, but doesn't seem to affect success.
        bestActionName = "Raid";
        reason = `Only remaining Operations. ${actionSummaryString(bestActionName)}`;
    } else {
        // Otherwise, pick the "highest-tier" action we can confidently perform, which should lead to the fastest rep-gain.
        ...
        let populationUncertain = candidateActions.some(
            (a) => maxChance(a) > options["success-threshold"] && minChance(a) < options["success-threshold"],
        );
        ...
        if (!populationUncertain)
            bestActionName = candidateActions.filter(
                (a) => minChance(a) > options["success-threshold"] && getCount(a) >= 1,
            )[0];
        // Special case for when population uncertainty is high - proceed so long as max chance is high enough
        else
            bestActionName = candidateActions.filter(
                (a) => maxChance(a) > options["success-threshold"] && getCount(a) >= 1,
            )[0];

        if (!bestActionName)
            // If there were none, allow us to fall-back to an action with a minimum chance >50%, and maximum chance > threshold
            bestActionName = candidateActions.filter(
                (a) => minChance(a) > 0.5 && maxChance(a) > options["success-threshold"] && getCount(a) >= 1,
            )[0];
        if (bestActionName)
            // If we found something to do, log details about its success chance range
        ...
        if (!bestActionName) {
            if (populationUncertain) {
                ...
                unreservedActions.every((a) => getCount(a) == 0) &&
                cityNames.every((c) => chaosByCity[c] < options["max-chaos"])
            ) {
                bestActionName = "Incite Violence";
                let [maxChaosCity, maxChaos] = getMaxKeyValue(chaosByCity, cityNames);
                reason =
                    `No work available, and max city chaos is ${maxChaos.toFixed(1)} in ${maxChaosCity}, ` +
                    `which is less than --max-chaos threshold ${options["max-chaos"]}`;
            } // Otherwise, consider training
            else if (
                unreservedActions.some((a) => maxChance(a) < options["success-threshold"]) && // Only if we aren't at 100% chance for everything
                staminaPct > options["high-stamina-pct"] &&
                timesTrained < options["training-limit"]
            ) {
                // Only if we have plenty of stamina and have barely trained
                ...
                reason =
                    `Nothing better to do, times trained (${timesTrained.toFixed(0)}) < --training-limit (${options["training-limit"]}), and ` +
                    `actions are below success threshold: ` +
                    unreservedActions
                        .filter((a) => maxChance(a) < options["success-threshold"])
                        .map((a) => `${a} (${(100 * maxChance(a)).toFixed(1)}%)`)
                        .join(", ");
            } else {
                // Otherwise, Field Analysis
    ...
    if (lastAssignedTask && lastAssignedTask != currentAction?.name && getCount(lastAssignedTask) > 0) {
        log(
            ns,
            `WARNING: The last task this script assigned was "${lastAssignedTask}", but you're now doing "${currentAction?.name || "(nothing)"}". ` +
                `Have you been using Bladeburner Automation? If so, try typing "automate dis" in the Bladeburner Console.`,
            false,
            "warning",
        );
    } else if (currentAction?.name) {
        const currentDuration = await getBBInfo(
            ns,
            `getActionTime(ns.args[0], ns.args[1])`,
            currentAction.type,
            currentAction.name,
        );
        if (!lastAssignedTask) {
            // Leave a log acknowledging if we just started up and there was an activity already underway.
            log(
                ns,
                `INFO: At startup, Bladeburner was already doing "${currentAction?.name}", ` +
                    (bestActionName != currentAction.name
                        ? `but we would prefer to do "${bestActionName}", so we will be switching.`
        ...
        if (currentDuration < currentTaskEndTime - Date.now()) {
            log(
                ns,
                `INFO: ${bestActionName == currentAction.name ? "Restarting" : "Cancelling"} action "${currentAction.name}" because its new duration ` +
    ...
    const bestActionType =
        nextBlackOp == bestActionName
            ? "Black Operations"
            : contractNames.includes(bestActionName)
              ? "Contracts"
              : operationNames.includes(bestActionName)
                ? "Operations"
                : "General";
    const success = await getBBInfo(ns, `startAction(ns.args[0], ns.args[1])`, bestActionType, bestActionName);
    const expectedDuration = await getBBInfo(
        ns,
        `getActionTime(ns.args[0], ns.args[1])`,
        bestActionType,
        bestActionName,
    ...
    log(
        ...
        !success,
        success ? (options["toast-operations"] ? "info" : undefined) : "error",
    );
...
async function switchToCity(ns, city, reason) {
    ...
    log(
        ns,
        (success ? "INFO: Switched" : "ERROR: Failed to switch") + ` to Bladeburner city "${city}" (${reason})`,
        !success,
        success ? (options["toast-relocations"] ? "info" : undefined) : "error",
    );
    return success;
}
...
async function spendSkillPoints(ns) {
    while (true) {
        // Loop until we determine there's nothing left to spend skill points on
        ...
        let skillToUpgrade,
            minPercievedCost = Number.MAX_SAFE_INTEGER;
        for (const skillName of skillNames) {
            ...
            if (percievedCost < minPercievedCost) [skillToUpgrade, minPercievedCost] = [skillName, percievedCost];
        }
        ...
        if (await getBBInfo(ns, `upgradeSkill(ns.args[0])`, skillToUpgrade))
            log(
                ns,
                `SUCCESS: Upgraded Bladeburner skill ${skillToUpgrade}`,
                false,
                options["toast-upgrades"] ? "success" : undefined,
            );
        else
            log(
                ns,
                `WARNING: Something went wrong while trying to upgrade Bladeburner skill ${skillToUpgrade}. ` +
                    `Currently have ${unspent} SP, upgrade should cost ${skillCosts[skillToUpgrade]} SP.`,
                false,
                "warning",
            );
        await ns.sleep(10);
...
async function canDoBladeburnerWork(ns) {
    ...
    if (lastCanWorkCheckIdle)
        log(
            ns,
            `WARNING: Cannot perform Bladeburner actions because the player is busy ` +
                `and hasn't installed the augmentation "${simulacrumAugName}"...`,
            false,
            "warning",
        );
...
async function beingInBladeburner(ns) {
    ...
    while (!(await getNsDataThroughFile(ns, "ns.bladeburner.inBladeburner()"))) {
        try {
            if (
                ...
                log(
                    ns,
                    `Waiting for physical stats >100 to join bladeburner ` +
                ...
                if (9 in ownedSourceFiles && options["disable-spending-hashes"])
                    message +=
                        " --disable-spending-hashes is set, but consider running the following command to give it a boost:\n" +
                        "run scripts/hacknet-spend.js --bladeburnerSP --bladeburnerRank";
                log(ns, message, true, "success");
                break;
            } else
                log(
                    ns,
                    "WARNING: Failed to joined Bladeburner despite physical stats. Will try again...",
                    false,
                    "warning",
                );
            player = await getNsDataThroughFile(ns, "ns.getPlayer()");
            ...
            log(
                ns,
                `WARNING: bladeburner.js Caught (and suppressed) an unexpected error while waiting to join bladeburner, but will keep going:\n` +
                    (typeof err === "string" ? err : err.message || JSON.stringify(err)),
                false,
                "warning",
            );
    ...
    const fPath = getFilePath("scripts/hacknet-spend.js");
    const args = ["--bladeburnerSP", "--bladeburnerRank"];
    if (ns.run(fPath, { preventDuplicates: true }, ...args))
        log(
            ns,
            `INFO: Launched '${fPath}' to gain Bladeburner Rank and Skill Points more quickly (Can be disabled with --disable-spending-hashes)`,
        );
    else log(ns, `WARNING: Failed to launch '${fPath}' (already running?)`);
}
