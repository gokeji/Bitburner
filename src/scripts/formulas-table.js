...
export async function main(ns) {
  ...
  ns.print("Security Diff".padEnd(20) + " | " + results.map(r => ("+" + r.securityDiff).padEnd(colWidth)).join(" | "));
  ...
  // Hack functions
  ns.print("\n--- HACK FUNCTIONS ---");
  ns.print("Hack Chance".padEnd(20) + " | " + results.map(r => fmtPercent(r.hackChance).padEnd(colWidth)).join(" | "));
  ns.print("Hack % (1 thread)".padEnd(20) + " | " + results.map(r => fmtPercent(r.hackPercent).padEnd(colWidth)).join(" | "));
  ns.print("Hack Exp".padEnd(20) + " | " + results.map(r => fmt(r.hackExp).padEnd(colWidth)).join(" | "));
  ns.print("Money/1 thread".padEnd(20) + " | " + results.map(r => {
    if (r.hackPercent === "N/A") return "N/A".padEnd(colWidth);
    return fmtMoney(r.money * r.hackPercent).padEnd(colWidth);
  }).join(" | "));
  ns.print(`Hack Sec (${testThreads}t)`.padEnd(20) + " | " + results.map(r => fmtSecurity(r.hackSecurityIncrease).padEnd(colWidth)).join(" | "));

  // Grow functions
  ns.print("\n--- GROW FUNCTIONS ---");
  ns.print(`Grow % (${testThreads} threads)`.padEnd(20) + " | " + results.map(r => {
    if (r.growPercent10 === "N/A") return "N/A".padEnd(colWidth);
    return (ns.formatNumber(r.growPercent10, 2) + "x").padEnd(colWidth);
  }).join(" | "));
  ns.print(`Grow Amount (${testThreads}t)`.padEnd(20) + " | " + results.map(r => fmtMoney(r.growAmount10).padEnd(colWidth)).join(" | "));
  ns.print("Threads to Max".padEnd(20) + " | " + results.map(r => ns.formatNumber(r.growThreadsToMax, 2).padEnd(colWidth)).join(" | "));
  ns.print("Threads to 80%".padEnd(20) + " | " + results.map(r => ns.formatNumber(r.growThreadsTo80, 2).padEnd(colWidth)).join(" | "));
  ns.print(`Grow Sec (${testThreads}t)`.padEnd(20) + " | " + results.map(r => fmtSecurity(r.growSecurityIncrease).padEnd(colWidth)).join(" | "));

  // Weaken functions
  ns.print("\n--- WEAKEN FUNCTIONS ---");
  ns.print("Weaken Amount (1t)".padEnd(20) + " | " + results.map(r => {
  ...
  if (Math.abs(current.hackPercent - formulasCurrent.hackPercent) > 0.0001) {
    ns.print(`⚠️  Hack percent differs: ${fmtPercent(current.hackPercent)} vs ${fmtPercent(formulasCurrent.hackPercent)}`);
  }

  if (Math.abs(current.growThreadsToMax - formulasCurrent.growThreadsToMax) > 1) {
    ns.print(`⚠️  Grow threads differ: ${fmt(current.growThreadsToMax)} vs ${fmt(formulasCurrent.growThreadsToMax)}`);
  }

  if (Math.abs(current.hackSecurityIncrease - formulasCurrent.hackSecurityIncrease) > 0.001) {
    ns.print(`⚠️  Hack security differs: ${fmtSecurity(current.hackSecurityIncrease)} vs ${fmtSecurity(formulasCurrent.hackSecurityIncrease)}`);
  }

  if (Math.abs(current.growSecurityIncrease - formulasCurrent.growSecurityIncrease) > 0.001) {
    ns.print(`⚠️  Grow security differs: ${fmtSecurity(current.growSecurityIncrease)} vs ${fmtSecurity(formulasCurrent.growSecurityIncrease)}`);
  }
  ...
  ns.print(`  Hack time: ${fmtTime(currentState.hackTime)} → ${fmtTime(currentPlus10.hackTime)} (${((currentPlus10.hackTime/currentState.hackTime - 1) * 100).toFixed(1)}% increase)`);
  ns.print(`  Grow time: ${fmtTime(currentState.growTime)} → ${fmtTime(currentPlus10.growTime)} (${((currentPlus10.growTime/currentState.growTime - 1) * 100).toFixed(1)}% increase)`);
  ns.print(`  Weaken time: ${fmtTime(currentState.weakenTime)} → ${fmtTime(currentPlus10.weakenTime)} (${((currentPlus10.weakenTime/currentState.weakenTime - 1) * 100).toFixed(1)}% increase)`);

  ns.print(`Security +10 impact on effectiveness:`);
  ns.print(`  Hack %: ${fmtPercent(currentState.hackPercent)} → ${fmtPercent(currentPlus10.hackPercent)} (${((currentPlus10.hackPercent/currentState.hackPercent - 1) * 100).toFixed(1)}% change)`);
  ns.print(`  Grow multiplier: ${currentState.growPercent10.toFixed(3)}x → ${currentPlus10.growPercent10.toFixed(3)}x (${((currentPlus10.growPercent10/currentState.growPercent10 - 1) * 100).toFixed(1)}% change)`);
