...
export async function main(ns) {
    ...
    for (const state of testStates) {
        ...
        ns.tprint(
            `  Money: $${ns.formatNumber(server.moneyAvailable)} / $${ns.formatNumber(server.moneyMax)} (${((server.moneyAvailable / server.moneyMax) * 100).toFixed(1)}%)`,
        );
        ns.tprint(
            `  Security: ${server.hackDifficulty.toFixed(2)} / ${server.minDifficulty.toFixed(2)} (+${(server.hackDifficulty - server.minDifficulty).toFixed(2)})`,
        );
