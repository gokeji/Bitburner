...
export async function main(ns) {
    ...
    let buttons = Array.from(
        doc
            .evaluate("//button[text()='Stop playing']", doc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null)
            .singleNodeValue.parentElement.children[6].getElementsByTagName("button"),
    )
        .map((x) => [parseInt(x.innerText), x])
        .filter((x) => x[0].toString() == x[1].innerText)
        .sort((a, b) => {
            return a[0] - b[0];
        });
    let wheels = [];
    ...
    while (!doc.body.innerText.includes("You're not allowed here anymore.")) {
        ...
        let wheels2 = wheels.filter(
            (x) =>
                levenshteinDistance(x.slice(0, seen.length - 1), seen.slice(0, seen.length - 1)) <
                Math.max(5, seen.length / 2),
        );
        ...
        seen.push(
            parseInt(
                doc.evaluate("//button[text()='Stop playing']", doc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null)
                    .singleNodeValue.parentElement.children[3].innerText,
            ),
        );
    }
}
