...
export async function main(ns) {
    ...
    for (const [symbol, data] of sortedStockEntries) {
        const priceForecastList = data.rawPrices
            .map((pair) => {
                if (pair[0] === "\n") return pair[0]; // Keep newlines as they are
                const [price, forecast] = pair;
                const formattedPrice = price >= 1000 ? `$${(price / 1000).toFixed(3)}k` : `$${price.toFixed(3)}`;
                return `${formattedPrice}/${forecast.toFixed(2)}`;
            })
            .join(" > ");

        const lowestPriceFormatted =
            data.lowestPrice >= 1000 ? (data.lowestPrice / 1000).toFixed(3) + "k" : data.lowestPrice.toFixed(3);
        const highestPriceFormatted =
            data.highestPrice >= 1000 ? (data.highestPrice / 1000).toFixed(3) + "k" : data.highestPrice.toFixed(3);
        const medianPriceFormatted =
            data.medianPriceRaw >= 1000
