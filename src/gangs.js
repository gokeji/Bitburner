...
const defaultMaxSpendPerTickTransientEquipment = 0.001; // If the --equipment-budget is not specified, spend up to this percent of non-reserved cash on temporary upgrades (equipment)
const defaultMaxSpendPerTickPermanentEquipment = 0.5; // If the --augmentation-budget is not specified, spend up to this percent of non-reserved cash on permanent member upgrades
...
async function initialize(ns) {
    ...
    while (!haveJoinedAGang) {
        try {
            ...
            if (bitNode == 2 || ns.heart.break() <= -54000)
                await runCommand(
                    ns,
                    `ns.args.forEach(g => ns.gang.createGang(g))`,
                    "/Temp/gang-createGang.js",
                    gangsByPower,
                );
        } catch (err) {
            log(
    ...
    if (sf4Level == 0)
        log(
            ns,
            `INFO: SF4 required to get gang augmentation info. Defaulting to assuming ~2.5 million rep is desired.`,
        );
    else {
        try {
            if (sf4Level < 3)
            ...
            const dictAugRepReqs = await getDict(
                ns,
                augmentationNames,
            ...
            requiredRep = augmentationNames
                .filter((aug) => !ownedAugmentations.includes(aug) && aug != "The Red Pill")
...
async function mainLoop(ns) {
    ...
    if (!territoryTickDetected) {
        ...
        if (lastOtherGangInfo != null && JSON.stringify(otherGangInfo) != JSON.stringify(lastOtherGangInfo)) {
            ...
            log(
                ns,
                `INFO: Others gangs power updated (sometime in the past ${formatDuration(thisLoopStart - lastLoopTime)}. ` +
                    `Will start waiting for next tick in: ${formatDuration(territoryNextTick - thisLoopStart - territoryTickWaitPadding)}`,
                false,
            );
    ...
    if (
        ...
        log(
            ns,
            `INFO: Waiting for territory to tick. (Waiting for gang power to change from ${formatNumberShort(lastTerritoryPower)}. ETA: ${formatDuration(territoryNextTick - thisLoopStart)}`,
        );
...
async function onTerritoryTick(ns, myGangInfo) {
    ...
    if (lastTerritoryPower != myGangInfo.power || lastTerritoryPower == null) {
        log(
            ns,
            `Territory power updated from ${formatNumberShort(lastTerritoryPower)} to ${formatNumberShort(myGangInfo.power)}.`,
        );
        ...
        log(
            ...
            false,
            consecutiveTerritoryDetections == 0 ? "warning" : null,
...
async function updateMemberActivities(ns, dictMemberInfo = null, forceTask = null, myGangInfo = null) {
    ...
    if (
        ...
        log(
            ns,
            `ERROR: Failed to set member task of one or more members: ` + JSON.stringify(workOrders),
            false,
            "error",
        );
}
...
async function optimizeGangCrime(ns, myGangInfo) {
    ...
    const optStat = options["reputation-focus"]
        ? "respect"
        : options["money-focus"]
          ? "money"
    ...
    if (optStat == "both money and respect") {
        Object.values(memberTaskRates)
            .flat()
            .forEach((v) => (v[optStat] = v.money / 1000 + v.respect)); // Hack to support a "optimized total" stat when trying to balance both money and wanted
    ...
    for (let shuffle = 0; shuffle < 100; shuffle++) {
            ...
            const bestTask =
                ...
                Date.now() - (lastMemberReset[member] || 0) < options["min-training-ticks"] * territoryTickTime
                    ...
                    : totalWanted > wantedGainTolerance || sustainableTasks.length == 0
                      ...
                      : sustainableTasks[0];
            [proposedTasks[member], totalWanted, totalGain] = [
                bestTask,
        ...
        while (
            ...
            const mostWanted = Object.keys(proposedTasks).reduce(
                (t, c) =>
            ...
            [proposedTasks[mostWanted], totalWanted, totalGain] = [
                nextBestTask,
                totalWanted + nextBestTask.wanted - proposedTasks[mostWanted].wanted,
                totalGain + nextBestTask[optStat] - proposedTasks[mostWanted][optStat],
            ];
    ...
    if (bestTaskAssignments != null && myGangMembers.some((m) => assignedTasks[m] !== bestTaskAssignments[m].name)) {
        ...
        const [optWanted, optRespect, optMoney] = myGangMembers
            .map((m) => assignedTasks[m])
            .reduce(([w, r, m], t) => [w + t.wanted, r + t.respect, m + t.money], [0, 0, 0]);
        if (
...
async function tryAscendMembers(ns) {
    ...
    for (let i = 0; i < myGangMembers.length; i++) {
        ...
        const ascMultiThreshold =
            options["ascend-multi-threshold"] + (11 - i) * options["ascend-multi-threshold-spacing"];
        const ascResult = dictAscensionResults[member];
        if (!ascResult || !importantStats.some((stat) => ascResult[stat] >= ascMultiThreshold)) continue;
        if (undefined !== (await getNsDataThroughFile(ns, `ns.gang.ascendMember(ns.args[0])`, null, [member]))) {
            log(
                ns,
                `SUCCESS: Ascended member ${member} to increase multis by ${importantStats.map((s) => `${s} -> ${ascResult[s].toFixed(2)}x`).join(", ")}`,
                false,
                "success",
            );
            lastMemberReset[member] = Date.now();
        } else log(ns, `ERROR: Attempt to ascended member ${member} failed. Go investigate!`, false, "error");
    }
}
...
async function tryUpgradeMembers(ns, dictMembers) {
    ...
    const dictEquipmentCosts = await getGangInfoDict(
        ns,
    ...
    let budget =
        Math.min(maxBudget, options["equipment-budget"] || defaultMaxSpendPerTickTransientEquipment) * homeMoney;
    let augBudget =
...
async function doUpgradePurchases(ns, purchaseOrder) {
    ...
    const succeeded = [],
        failed = [];
    for (let i = 0; i < orderOutcomes.length; i++) (orderOutcomes[i] ? succeeded : failed).push(purchaseOrder[i]);
...
async function waitForGameUpdate(ns, oldGangInfo) {
    ...
    log(
        ...
        false,
        sequentialMisfires < 2 ? null : "warning",
...
const getGangInfoDict = /**@returns{Promise<{[gangMember: string]: any;}>}*/ async (ns, elements, gangFunction) =>
    await getDict(ns, elements, `gang.${gangFunction}`, `/Temp/gang-${gangFunction}.txt`);
const getDict = /**@returns{Promise<{[key: string]: any;}>}*/ async (ns, elements, nsFunction, fileName) =>
    await getNsDataThroughFile(
        ns,
