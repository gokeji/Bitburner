...
export default class BasePlayer {
    ...
    listGetters(instance, properties = new Set()) {
        let getters = Object.entries(Object.getOwnPropertyDescriptors(Reflect.getPrototypeOf(instance)))
    ...
    get hacking() {
        return {
            ...
            next_level_exp:
                Math.pow(Math.E, (this.data.hacking + 1) / (32 * this.data.hacking_mult) + 25 / 4) - 1069 / 2,
            tnl:
                Math.pow(Math.E, (this.data.hacking + 1) / (32 * this.data.hacking_mult) + 25 / 4) -
                1069 / 2 -
                this.data.hacking_exp,
            multipliers: {
    ...
    get hnet() {
        return {
            multipliers: this.ns.getHacknetMultipliers(),
        };
    }
    get market() {
        return {
    ...
    get ports() {
        return this.ns
            .ls("home")
            .filter((file) =>
                ["BruteSSH.exe", "FTPCrack.exe", "relaySMTP.exe", "HTTPWorm.exe", "SQLInject.exe"].includes(file),
            ).length;
    }
