...
function ascend(ns) {
    ...
    for (let member of members) {
        ...
        let memberAscensionMultiplier =
            (memberInfo.agi_asc_mult + memberInfo.def_asc_mult + memberInfo.dex_asc_mult + memberInfo.str_asc_mult) / 4;
        ...
        if (memberAscensionResult != undefined) {
            let memberAscensionResultMultiplier =
...
function assignMembers(ns, territoryWinChance) {
    ...
    for (let member of members) {
        ...
        if (workJobs > 0 && gangInfo.territory < 1 && members.length >= 12 && territoryWinChance < 0.95) {
            ...
            wantedLevelIncrease += ns.formulas.gang.wantedLevelGain(
                gangInfo,
                ns.gang.getMemberInformation(member),
                ns.gang.getTaskStats(highestValueTask),
            );
...
function taskValue(ns, gangInfo, member, task) {
    ...
    let respectGain = ns.formulas.gang.respectGain(
        gangInfo,
        ns.gang.getMemberInformation(member),
        ns.gang.getTaskStats(task),
    );
    let moneyGain = ns.formulas.gang.moneyGain(
        gangInfo,
        ns.gang.getMemberInformation(member),
        ns.gang.getTaskStats(task),
    );
    let wantedLevelIncrease = ns.formulas.gang.wantedLevelGain(
        gangInfo,
        ns.gang.getMemberInformation(member),
        ns.gang.getTaskStats(task),
    );
    let vigilanteWantedDecrease = ns.formulas.gang.wantedLevelGain(
        gangInfo,
        ns.gang.getMemberInformation(member),
