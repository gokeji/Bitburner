    ...
    if (player.workType == "Working for Faction") {
        if (factions.has(player.currentWorkFactionName)) {
            ...
            if (repRemaining > 0) {
                ...
                if (
                    playerControlPort.empty() &&
                    player.currentWorkFactionDescription == "carrying out hacking contracts"
                ) {
                ...
                ns.print(
                    "Reputation remaining: " +
                        ns.nFormat(repRemaining, "0a") +
                        " in " +
                        ns.nFormat(reputationTimeRemaining / 60, "0a") +
                        " min",
                );
                return true;
            } else {
...
function buyAugments(ns, player) {
    ...
    for (var i = 0; i < sortedAugmentations.length; i++) {
        ...
        if (i >= 0) {
            if (
                (i > 0 && sortedAugmentations[i][0] == sortedAugmentations[i - 1][0]) ||
                skipAugments.includes(sortedAugmentations[i][0])
            ) {
