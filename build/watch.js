...
import { src, dist, allowedFiletypes, excludedFiles } from "./config.js";
...
function filterBitBurnerImports(content, filePath) {
    ...
    if (ext === ".js" || ext === ".ts") {
        ...
        return content
            .replace(/^\/\/\s*import\s*\{\s*NS\s*\}\s*from\s*["']@ns["']\s*;?\s*$/gm, "")
...
async function syncStatic() {
    return syncDirectory.async(path.resolve(src), path.resolve(dist), {
        exclude: (file) => {
            ...
            return (ext && !allowedFiletypes.includes(ext)) || excludedFiles.some((pattern) => file.includes(pattern));
        },
